...
- **メインスレ巡回の Actions 分離**: `scrape.yml`(地域別巡回)は `SCRAPER_SKIP_DIRECT=1` で directThreads をスキップし、`scrape-main.yml`(メインスレ専用・10 分おき cron)が `SCRAPER_DIRECT_ONLY=1` で **directThreads のみ**を巡回する(index.mjs はキーワード検索・一覧巡回を省いて directThreads だけを取得。未設定なら何もせず正常終了)。通知フローは両方とも notify.mjs の gist + URL 通知のまま。**状態キャッシュは別キー**(`scrape-state-v1-` / `scrape-main-state-v1-`)— 担当スレが重複しない設計のため基準の分離で二重通知は起きない。`scrape-main.yml` の実行統計は Actions ログでのみ確認(log/latest-run.md には書かない)。**GitHub Actions の schedule は間引きされることがあるため高頻度 cron でも起動がスキップされ得る**(30 分以内の検知は保証されない)。これらの env なしのローカル実行は従来どおり一覧 + directThreads の両方を巡回する
