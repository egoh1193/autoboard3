...
- **schedule 間引き対策の Cloudflare Cron 起動**: Worker に `scheduled` ハンドラ(wrangler.jsonc の `triggers.crons` = `*/10`)があり、GitHub API の `workflow_dispatch` で `scrape.yml` を確実に起動する(Actions: Write 権限の PAT = リポジトリシークレット `GH_DISPATCH_TOKEN` → deploy.yml が Worker secret に同期。**未設定なら cron はスキップ**。dispatch 先リポジトリは `GH_DISPATCH_REPO`(未設定なら既定値)。失敗時のログはステータスコードのみ)。`scrape-main.yml` は dispatch しない(schedule と併存。必要なら同様に追加)
