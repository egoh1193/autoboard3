...
巡回は 4 段階: ①(オプション)環境変数 `SCRAPER_KEYWORDS`(カンマ区切り)があればキーワードごとにスレ検索 URL(`targetUrl` + `search` ブロックのパラメータ)を組み立て、なければ(オプション)`categoryList` で掲示板一覧を列挙 — どちらも未設定なら `targetUrl` を直接スレ一覧として扱う ②スレ一覧(`threadList.nextPage` + `maxPages` でページング、キーワード間の重複は ID で除去) ③`filters.titleIncludes/titleExcludes` でタイトル絞り込み ④該当スレ本文(`thread.nextPage` + `maxPages` でスレッド内ページ送り)→ レス配列(性別排除: `filters.sexExcludes` / 環境変数 `SCRAPER_SEX_EXCLUDES`(カンマ区切り・部分一致)に該当するレスを出力から除外。`isSexExcluded()` は parse.mjs 共有、Worker も同じく排除)。**`config.directThreads`(配列)で一覧を経由しない「メインスレ」を直接指定できる**: 要素は URL 文字列 or `{url, title, newestFirst, maxPages, maxAgeDays}`(`newestFirst: true` は p=1 が最新の降順ページングのスレ。`maxPages` / `maxAgeDays` は**スレ単位の上限上書き** — 重たいスレを個別に軽量化でき、未指定なら共通値 `thread.maxPages` / `thread.maxAgeDays` を使う。`maxAgeDays: 0` で範囲制限なし)。**別エンジンの板(サブ掲示板)もここに混在できる**: 要素に `id`(スレ ID の明示指定。id クエリを持たない板ページ URL 向け。未指定なら threadIdFromUrl)/ `mockFile`(モックモード時の mock/ 以下の HTML ファイル名)/ `category`(一覧のカテゴリ表示名)/ `thread`(板ごとのパーサ設定。オブジェクト=インライン or `config.threadConfigs` の名前参照)を指定できる。投稿 ID の無い板(1 枚板。`thread.postsSelector` + `fields` + `fieldPatterns` + `imagesSpec` + `keySpec`/`keyPattern` で構成。`keySpec` は通報・削除リンクの URL から投稿 ID `post.key` を取る)は、`thread.nextPage` リンクでページ送りし(ページ番号が URL パスセグメントの板は pageParam で組み立てられないため)、日時の新しい順に並べて `num` を振り直す(1 = 最新)。`fields` の値が `mailto:` 直リンクの板は接頭辞を剥がして `post.email` が確定する(個別メールページが存在しないためメールページ取得は行わない)。明示指定のためタイトルフィルタは適用されず、一覧由来スレと ID 重複時は一覧側を優先。**Worker も対応**(デプロイ時に deploy.yml が設定 gist から directThreads を抽出し Worker secret `SCRAPER_DIRECT_THREADS` に同期。Worker は一覧とは別の `mainThreads` として返し、フロントの一覧下部「メインスレ」に表示する。一覧と ID 重複時は取得済み詳細を流用)
...
- モック(すべて実物サンプル): `sample.html`=掲示板一覧(categoryList 使用時のみ)/ `sample-thread-list.html`=スレ一覧(大阪・梅田板)/ `sample-thread.html`=スレ本文(梅田スレ)/ `sample-panel-board.html`=サブ掲示板の 1 枚板(bootstrap パネル、投稿 ID は通報リンクの URL。directThreads 要素の `mockFile` で切り替え)/ `sample-mail.html`=個別ページ(メール送信)
...
- **巡回設定 gist**(`SCRAPER_SETTINGS_GIST_URL`, シークレット): gist 内の最初の `.json` ファイルに `{"keywords": [...], "sexExcludes": [...], "blackList": [...], "mirrorUrl": "…", "directThreads": [...], "threadConfigs": {...}, "post": {...}}` を書いておくと、各設定が未設定時に読み込まれる(優先順: 環境変数 > gist > config。`directThreads` は gist があれば gist 優先。`threadConfigs` は config 側に名前マージされる)。**`mirrorUrl` は系統①のミラー URL(notify が gist の元スレに併記するミラーリンクの基準)。`MIRROR_URL` シークレットがあればそちらが優先**。構造の全体像は `scraper/settings.sample.jsonc`(コメント付きサンプル。gist に貼る際はコメントを削除して正しい JSON にする)を参照。`directThreads` は一覧を経由しない「メインスレ」の直接指定(要素は URL 文字列 or `{url, title, newestFirst}`。URL は config.targetUrl 基準で解決されるため相対パス `/public/thread/index?id=N` だけでもよい)。`blackList` は**オブジェクト形式(`{ "mailList": [...], "keywordList": [...] }`)または配列形式(従来。文字列 or `{ mail, keyword }` の混合配列)で書ける** — `mailList` はメールアドレス完全一致(大文字小文字・空白は無視)、`keywordList` は文字列(本文部分一致)か `{ "keyword": "...", "mail": "..." }` オブジェクト(`keyword` は**投稿本文への部分一致**。大文字小文字無視。メールだけ変えて同じ広告を書く投稿者を落とすための投稿キーワード。`mail` 併記は AND 条件)— **メールアドレスは個別メールページ取得後に確定するためバッチ専用**(Worker はメールページを取得しない)。秘密 gist を読むため `GIST_TOKEN` が必要。**gist URL・ID・内容はログに出さない**。読み込み失敗時は実行を中止する(意図しないフィルタでの巡回・通知を避ける)。`directThreads` は Worker にもデプロイ時に同期される(deploy.yml → Worker secret `SCRAPER_DIRECT_THREADS`)
- `notify.mjs`: 前回実行の状態(`.scrape-state.json`)と差分し、新着レス(レス全文・付帯情報・メールアドレス)を**投稿者ごと**にまとめた Markdown を**秘密 gist** に投稿し、その URL だけを Discord に投稿する(本文を Discord に直接は送らない)。各レスの見出しに元投稿スレ(タイトル・URL)を付与(`buildGistContent()`)。**ミラー URL を設定すると元スレにミラーリンク(`{mirrorUrl}/thread?id=…`)を併記する。基準の優先順: ①リポジトリシークレット `MIRROR_URL` ②自動解決(scrape.yml / scrape-main.yml の Resolve mirror URL ステップが、シークレット未設定のとき Cloudflare API から workers.dev サブドメインを取得し wrangler.jsonc の Worker 名で URL を組み立てる)③設定 gist の `mirrorUrl`(notify が未設定時に読む)**(いずれも未設定なら出さない。リポジトリは public のためミラー URL はコード・ログに埋め込まない)。**差分はレス単位**: 状態には `knownIds`(既知スレ ID)に加えて `knownPosts`(スレごとの既知最終レス番号)を保存し、**新規スレは取得窓内の全レス・既存スレは既知の最終レス番号より大きいレス番号のレス**が新着になる(旧形式の knownIds のみの状態ファイルもそのまま読める)。**レス番号の無い板(サブ掲示板の 1 枚板。全投稿に投稿 ID `post.key` があるスレ)は `knownKeys`(スレごとの既知投稿 ID 集合・上限 500 件で刈り込み)で差分を取る** — num は実行ごとに日時順で振り直されるため番号差分は使えない。基準は単調に進めるため、レス窓の関係で 1 回だけレスが取れない実行があっても同じレスを重複通知しない。**本文が空 or 2 文字以下のレスは募集削除の可能性が高いため gist には載せない**(基準の進行には含むため再通知もされない)。本文未取得スレ(詳細ファイルなし)は基準を進めない → 次回に新着として通知される。`DISCORD_WEBHOOK_URL` or `GIST_TOKEN` 未設定なら状態を更新しない(設定後に通知される設計)。状態ファイルは actions/cache で次回実行へ引き継ぐ。`GIST_TOKEN` は gist 権限を持つ PAT(Actions 既定の GITHUB_TOKEN では gist 作成不可)
...
- **メインスレ巡回の Actions 分離**: `scrape.yml`(地域別巡回)は `SCRAPER_SKIP_DIRECT=1` で directThreads をスキップし、`scrape-main.yml`(メインスレ専用・schedule は 30 分おき)が `SCRAPER_DIRECT_ONLY=1` で **directThreads のみ**を巡回する(index.mjs はキーワード検索・一覧巡回を省いて directThreads だけを取得。未設定なら何もせず正常終了)。通知フローは両方とも notify.mjs の gist + URL 通知のまま。**状態キャッシュは別キー**(`scrape-state-v1-` / `scrape-main-state-v1-`)— 担当スレが重複しない設計のため基準の分離で二重通知は起きない。Discord 通知の見出しは `SCRAPER_DIRECT_ONLY=1` のとき「メインスレの更新がありました。」、それ以外は「更新がありました。」(buildDiscordMessage)`scrape-main.yml` の実行統計は Actions ログでのみ確認(log/latest-run.md には書かない)。**GitHub Actions の schedule は間引きされることがあるため高頻度 cron でも起動がスキップされ得る**(30 分以内の検知は保証されない)。これらの env なしのローカル実行は従来どおり一覧 + directThreads の両方を巡回する
- **schedule 間引き対策の Cloudflare Cron 起動**: Worker に `scheduled` ハンドラ(wrangler.jsonc の `triggers.crons` = `*/10`)があり、GitHub API の `workflow_dispatch` で `scrape.yml`(地域別巡回)と `scrape-main.yml`(メインスレ専用)を確実に起動する(Actions: Write 権限の PAT = リポジトリシークレット `GH_DISPATCH_TOKEN` → deploy.yml が Worker secret に同期。**未設定なら cron はスキップ**。dispatch 先リポジトリは `GH_DISPATCH_REPO`(未設定なら既定値)。失敗時のログはステータスコードのみ)
- `scrape.yml` は**デプロイしない**。デプロイは `deploy.yml`(コード変更時)のみ
...
`CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` / `SCRAPER_CONFIG` / `GIST_TOKEN`(gist 権限の PAT)/ `DISCORD_WEBHOOK_URL` / `MIRROR_URL`(系統②の通知に併記するミラー URL)/ `GH_DISPATCH_TOKEN`(Actions: Write 権限の PAT。Worker cron が scrape.yml を dispatch する用)。デプロイ時、`SCRAPER_CONFIG` / `GH_DISPATCH_TOKEN` 等は Worker secret として自動同期される。